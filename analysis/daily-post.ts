...
function generateDailyPost(): void {
  ...
  const prev1500 = prevData.analyses[3] as ExtendedAnalysis;
  ...
  console.log('𝗧𝗼𝗽 𝟭𝟬𝟬: ' + current100.averages.gain.toFixed(1) + '% YTD (' +
    formatPercentage(perfChange100) + ' daily)');
  console.log('𝗕𝗿𝗼𝗮𝗱 𝗚𝗿𝗼𝘂𝗽: ' + current1500.averages.gain.toFixed(1) + '% YTD (' +
    formatPercentage(perfChange1500) + ' daily)');
  console.log('𝗧𝗼𝗽 𝟭𝟬𝟬 𝗮𝗱𝘃𝗮𝗻𝘁𝗮𝗴𝗲: ' + formatPercentage(current100.averages.gain - current1500.averages.gain, 1) + 'pp');
  ...
  const copierChanges = findTopCopierChanges(currentData.investors, prevData.investors, 1);
  ...
  if (Math.abs(avgCashChange) > 1) {
    ...
    console.log('• Significant daily ' + direction + ' - portfolios moving ' + formatPercentage(Math.max(Math.abs(perfChange100), Math.abs(perfChange1500))));
