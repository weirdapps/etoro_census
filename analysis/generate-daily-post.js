...
function generateSmartTakeaway(fearGreed, cashChange100, gainGap, allMovers) {
  ...
  if (fearGreed.status === 'Extreme Fear' && Math.abs(cashChange100) > 0.5) {
    return 'Elite investors in full defensive mode - high cash levels signal deep market caution';
  } else if (fearGreed.status === 'Extreme Greed' && cashChange100 < -0.5) {
    return 'Risk-on euphoria as elite investors deploy capital aggressively';
  } else if (Math.abs(gainGap) > 4) {
    return 'Performance gap ' + (gainGap > 0 ? 'widening' : 'narrowing') + ' - elite skill advantage ' + (gainGap > 0 ? 'expanding' : 'diminishing');
...
function generateDailyPost() {
  ...
  if (Math.abs(cashChange100 - cashChange1500) > 0.5) {
    ...
    insight = 'Elite investors turning ' + direction + ' than broad market - divergent risk appetite emerging';
    ...
    insight = 'Active portfolio reshuffling across both elite and broad investor groups';
  } else if (Math.abs(gainGap) > 4) {
    insight = 'Elite advantage remains strong at ' + gainGap.toFixed(1) + 'pp - skill gap persistent';
  } else {
    insight = 'Synchronized behavior between elite and broad investors - market consensus evident';
  }
